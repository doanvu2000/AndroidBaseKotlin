    ...
    private void applyRepeatingRequestBuilder(boolean checkStarted, int errorReason) {
        if ((getState() == CameraState.PREVIEW && !isChangingState()) || !checkStarted) {
            try {
                ...
                LOG.e("applyRepeatingRequestBuilder: session is invalid!", e, "checkStarted:", checkStarted, "currentThread:", Thread.currentThread().getName(), "state:", getState(), "targetState:", getTargetState());
//                throw new CameraException(CameraException.REASON_DISCONNECTED);
        ...
        try {
            ...
            mManager.openCamera(mCameraId, new CameraDevice.StateCallback() {
                ...
                public void onDisconnected(@NonNull CameraDevice camera) {
                    ...
                    CameraException exception = new CameraException(CameraException.REASON_DISCONNECTED);
    ...
    protected Task<Void> onStartBind() {
        ...
        if (outputClass == SurfaceHolder.class) {
            try {
                ...
                Tasks.await(Tasks.call((Callable<Void>) () -> {
        ...
        if (hasFrameProcessors()) {
            ...
            mFrameProcessingReader = ImageReader.newInstance(mFrameProcessingSize.getWidth(), mFrameProcessingSize.getHeight(), mFrameProcessingFormat, getFrameProcessingPoolSize() + 1);
        ...
        try {
            ...
            mCamera.createCaptureSession(outputSurfaces, new CameraCaptureSession.StateCallback() {
                ...
                public void onConfigureFailed(@NonNull CameraCaptureSession session) {
                    ...
                    if (!task.getTask().isComplete()) {
                        task.trySetException(new CameraException(cause, CameraException.REASON_FAILED_TO_START_PREVIEW));
                    } else {
    ...
    protected Task<Void> onStartPreview() {
        ...
        if (mFullVideoPendingStub != null) {
            ...
            getOrchestrator().scheduleStateful("do take video", CameraState.PREVIEW, () -> doTakeVideo(stub));
        }
    ...
    public void onPictureResult(@Nullable PictureResult.Stub result, @Nullable Exception error) {
        ...
        boolean unlock = (fullPicture && getPictureMetering()) || (!fullPicture && getPictureSnapshotMetering());
        if (unlock) {
            getOrchestrator().scheduleStateful("reset metering after picture", CameraState.PREVIEW, this::unlockAndResetMetering);
        }
    }
    ...
    public void onVideoRecordingEnd() {
        ...
        boolean needsIssue549Workaround = (mVideoRecorder instanceof Full2VideoRecorder) && (readCharacteristic(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL, -1) == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY);
    ...
    public void onVideoResult(@Nullable VideoResult.Stub result, @Nullable Exception exception) {
        ...
        getOrchestrator().scheduleStateful("restore preview template", CameraState.BIND, this::maybeRestorePreviewTemplateAfterVideo);
    }
    ...
    private void maybeRestorePreviewTemplateAfterVideo() {
        CaptureRequest captureRequest = mRepeatingRequestBuilder.build();
        if (captureRequest.getTag() != null) {
            int template = (int) captureRequest.getTag();
    ...
    protected void applyDefaultFocus(@NonNull CaptureRequest.Builder builder) {
        ...
        if (getMode() == Mode.VIDEO && modes.contains(CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_VIDEO)) {
    ...
    public void setFlash(@NonNull final Flash flash) {
        ...
        mFlash = flash;
        mFlashTask = getOrchestrator().scheduleStateful("flash (" + flash + ")", CameraState.ENGINE, () -> {
    ...
    protected boolean applyFlash(@NonNull CaptureRequest.Builder builder, @NonNull Flash oldFlash) {
        if (mCameraOptions.supports(mFlash)) {
            int[] availableAeModesArray = readCharacteristic(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES, new int[]{});
    ...
    public void setLocation(@Nullable Location location) {
        ...
        mLocation = location;
        mLocationTask = getOrchestrator().scheduleStateful("location", CameraState.ENGINE, () -> {
    ...
    @SuppressWarnings("WeakerAccess")
    protected boolean applyLocation(@NonNull CaptureRequest.Builder builder, @SuppressWarnings("unused") @Nullable Location oldLocation) {
        if (mLocation != null) {
    ...
    public void setWhiteBalance(@NonNull WhiteBalance whiteBalance) {
        ...
        mWhiteBalanceTask = getOrchestrator().scheduleStateful("white balance (" + whiteBalance + ")", CameraState.ENGINE, () -> {
    ...
    public void setHdr(@NonNull Hdr hdr) {
        final Hdr old = mHdr;
        mHdr = hdr;
        mHdrTask = getOrchestrator().scheduleStateful("hdr (" + hdr + ")", CameraState.ENGINE, () -> {
    ...
    public void setZoom(final float zoom, final @Nullable PointF[] points, final boolean notify) {
        ...
        mZoomTask = getOrchestrator().scheduleStateful("zoom", CameraState.ENGINE, () -> {
    ...
    public void setExposureCorrection(final float EVvalue, @NonNull final float[] bounds, @Nullable final PointF[] points, final boolean notify) {
        ...
        mExposureCorrectionTask = getOrchestrator().scheduleStateful("exposure correction", CameraState.ENGINE, () -> {
    ...
    protected boolean applyExposureCorrection(@NonNull CaptureRequest.Builder builder, float oldEVvalue) {
        if (mCameraOptions.isExposureCorrectionSupported()) {
            Rational exposureCorrectionStep = readCharacteristic(CameraCharacteristics.CONTROL_AE_COMPENSATION_STEP, new Rational(1, 1));
    ...
    public void setPreviewFrameRate(float previewFrameRate) {
        ...
        mPreviewFrameRateTask = getOrchestrator().scheduleStateful("preview fps (" + previewFrameRate + ")", CameraState.ENGINE, () -> {
    ...
    protected boolean applyPreviewFrameRate(@NonNull CaptureRequest.Builder builder, float oldPreviewFrameRate) {
        ...
        Range<Integer>[] fpsRanges = readCharacteristic(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES, new Range[]{});
    ...
    private void sortFrameRateRanges(@NonNull Range<Integer>[] fpsRanges) {
        ...
        Arrays.sort(fpsRanges, (range1, range2) -> {
            if (ascending) {
                return (range1.getUpper() - range1.getLower()) - (range2.getUpper() - range2.getLower());
            } else {
                return (range2.getUpper() - range2.getLower()) - (range1.getUpper() - range1.getLower());
            }
        });
    }
    ...
    public void setPictureFormat(final @NonNull PictureFormat pictureFormat) {
        if (pictureFormat != mPictureFormat) {
            ...
            getOrchestrator().scheduleStateful("picture format (" + pictureFormat + ")", CameraState.ENGINE, this::restart);
        }
    }
    ...
    public void setHasFrameProcessors(final boolean hasFrameProcessors) {
        ...
        getOrchestrator().schedule("has frame processors (" + hasFrameProcessors + ")", true, () -> {
    ...
    public void setFrameProcessingFormat(final int format) {
        ...
        getOrchestrator().schedule("frame processing format (" + format + ")", true, () -> {
            if (getState().isAtLeast(CameraState.BIND) && isChangingState()) {
                // Extremely rare case in which this was called in between startBind and
                // startPreview. This can cause issues. Try later.
    ...
    @Override
    public void startAutoFocus(@Nullable final Gesture gesture, @NonNull final MeteringRegions regions, @NonNull final PointF legacyPoint) {
        ...
        getOrchestrator().scheduleStateful("autofocus (" + gesture + ")", CameraState.PREVIEW, () -> {
            ...
            wrapper.addCallback(new CompletionCallback() {
                ...
                protected void onActionCompleted(@NonNull Action a) {
                    getCallback().dispatchOnFocusEnd(gesture, action.isSuccessful(), legacyPoint);
                    getOrchestrator().remove("reset metering");
                    if (shouldResetAutoFocus()) {
                        getOrchestrator().scheduleStatefulDelayed("reset metering", CameraState.PREVIEW, getAutoFocusResetDelay(), () -> unlockAndResetMetering());
                    }
                }
            });
        });
    ...
    private void unlockAndResetMetering() {
        ...
        }, new MeterResetAction()).start(Camera2Engine.this);
    }
