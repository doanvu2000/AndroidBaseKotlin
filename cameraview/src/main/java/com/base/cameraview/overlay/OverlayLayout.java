...
public class OverlayLayout extends FrameLayout implements Overlay {
        ...
        synchronized (this) {
            ...
            switch (target) {
                ...
                case PICTURE_SNAPSHOT:
                    ...
                    LOG.v("draw", "target:", target, "canvas:", canvas.getWidth() + "x" + canvas.getHeight(), "view:", getWidth() + "x" + getHeight(), "widthScale:", widthScale, "heightScale:", heightScale, "hardwareCanvasMode:", mHardwareCanvasEnabled);
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (params.drawsOn(currentTarget)) {
            LOG.v("Performing drawing for view:", child.getClass().getSimpleName(), "target:", currentTarget, "params:", params);
            return doDrawChild(canvas, child, drawingTime);
        } else {
            LOG.v("Skipping drawing for view:", child.getClass().getSimpleName(), "target:", currentTarget, "params:", params);
            return false;
        }
    }
    ...
    public static class LayoutParams extends FrameLayout.LayoutParams {
        ...
        boolean drawsOn(@NonNull Target target) {
            return ((target == Target.PREVIEW && drawOnPreview) || (target == Target.VIDEO_SNAPSHOT && drawOnVideoSnapshot) || (target == Target.PICTURE_SNAPSHOT && drawOnPictureSnapshot));
        }

        @NonNull
        @Override
        public String toString() {
            return getClass().getName() + "[" + "drawOnPreview:" + drawOnPreview + ",drawOnPictureSnapshot:" + drawOnPictureSnapshot + ",drawOnVideoSnapshot:" + drawOnVideoSnapshot + "]";
        }
    }
}
