...
import com.base.simplecropview.animation.SimpleValueAnimator;
import com.base.simplecropview.animation.SimpleValueAnimatorListener;
import com.base.simplecropview.animation.ValueAnimatorV14;
import com.base.simplecropview.callback.Callback;
import com.base.simplecropview.callback.CropCallback;
import com.base.simplecropview.callback.SaveCallback;
import com.base.simplecropview.util.Logger;
import com.base.simplecropview.util.Utils;
...
public class CropImageView extends ImageView {
    ...
    private void drawDebugInfo(Canvas canvas) {
        ...
        if (mSourceUri == null) {
            builder.append("INPUT_IMAGE_SIZE: ").append((int) mImgWidth).append("x").append((int) mImgHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder();
        } else {
            builder = new StringBuilder().append("INPUT_IMAGE_SIZE: ").append(mInputImageWidth).append("x").append(mInputImageHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder();
        }
        ...
        if (mOutputImageWidth > 0 && mOutputImageHeight > 0) {
            builder.append("OUTPUT_IMAGE_SIZE: ").append(mOutputImageWidth).append("x").append(mOutputImageHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
        ...
        if (mIsAnimationEnabled) {
            ...
            animator.addAnimatorListener(new SimpleValueAnimatorListener() {
                ...
                public void onAnimationUpdated(float scale) {
                    mFrameRect = new RectF(currentRect.left + diffL * scale, currentRect.top + diffT * scale, currentRect.right + diffR * scale, currentRect.bottom + diffB * scale);
                    invalidate();
                }
    ...
    private float getRatioX(float w) {
        return switch (mCropMode) {
            case FIT_IMAGE -> mImageRect.width();
            case FREE -> w;
            case RATIO_4_3 -> 4;
            case RATIO_3_4 -> 3;
            case RATIO_16_9 -> 16;
            case RATIO_9_16 -> 9;
            case SQUARE, CIRCLE, CIRCLE_SQUARE -> 1;
            case CUSTOM -> mCustomRatio.x;
        };
    }

    private float getRatioY(float h) {
        return switch (mCropMode) {
            case FIT_IMAGE -> mImageRect.height();
            case FREE -> h;
            case RATIO_4_3 -> 3;
            case RATIO_3_4 -> 4;
            case RATIO_16_9 -> 9;
    ...
    private Bitmap getRotatedBitmap(Bitmap bitmap) {
        ...
        rotateMatrix.setRotate(mAngle, (float) bitmap.getWidth() / 2, (float) bitmap.getHeight() / 2);
    ...
    private Bitmap getCroppedBitmapFromUri() throws IOException {
        ...
        try {
            inputStream = getContext().getContentResolver().openInputStream(mSourceUri);
